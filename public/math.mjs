...
async function clearOldServiceWorker() {
  // Clear all databases with scramjet in name
  try {
    const databases = await indexedDB.databases();
    for (const db of databases) {
      if (db.name && db.name.includes('scramjet')) {
        console.log('Clearing old database:', db.name);
...
// Always clean on first load or when flagged
const swInitialized = sessionStorage.getItem('sw-initialized');
const needsReset = localStorage.getItem('sw-needs-reset');

if (!swInitialized || needsReset === 'true') {
  await clearOldServiceWorker();
  sessionStorage.setItem('sw-initialized', 'true');
  localStorage.removeItem('sw-needs-reset');
  console.log('Service worker cleaned and initialized');
}

//////////////////////////////
///         Options        ///
//////////////////////////////
// Initialize BareMux connection - MUST be created before any service worker interactions
...
await connection.getTransport().catch(() => null);
...
async function registerSW() {
	...
	try {
		// Wait for existing service workers to be ready
		...
		// Mark for reset on next load
		localStorage.setItem('sw-needs-reset', 'true');
		throw error;
	}
}
...
if (window.self === window.top) {
	...
	// Set default transport and wisp AFTER service worker is ready
	await setTransport("epoxy");
	await setWisp(`ws://${location.host}/wisp/`);
}
